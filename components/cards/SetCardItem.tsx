...
const VARIANT_KEYS = ["normal", "holo", "reverse", "firstEdition", "wPromo"] as const;

export default function SetCardItem({ card, setId, standalone = false, controlSize = "sm" }: Props) {
    ...
    const cardVisual = (
        <div className={`bg-white border border-wisteria rounded-xl overflow-hidden transition-all ${totalOwned > 0 ? "opacity-100" : "opacity-50 hover:opacity-100"}`}>
            <div className="bg-lavender relative">
                ...
                {!standalone && (
                    <div className="flex flex-row gap-1 absolute bottom-0 px-2.5 py-1 bg-iris text-midnight rounded-tr-xl">
                        <p className="text-[12px]">{card.localId}/{card.set.cardCount.official}</p>
    ...
    return (
        <div key={card.id} className="flex flex-col">
            ...
            <div className="flex flex-col items-center justify-between mt-1 px-0.5 relative">
                ...
                <div className={`flex flex-row align-middle justify-between w-full px-5 ${isLg ? "mt-2" : "mt-0.5"}`}>
                    ...
                    <div className="flex items-center gap-1.5">
                        <button
                            ...
                            className={`text-heather hover:text-violet transition-colors ${isLg ? "text-xl w-8 h-8" : "text-sm w-5 h-5"} flex items-center justify-center`}
                        >-</button>
                        <span className={`font-medium text-midnight ${isLg ? "text-xl w-8 h-8" : "text-sm w-5 h-5"} text-center`}>
                            {defaultOwned}
                        </span>
                        <button
